...
4. Fachada de produto (`src/features/ascii-engine`)
   ...
   - centraliza módulos como `converters`, `gallery`, `themes`, `presets`, `exporters`, `importers`, `document`, `storage`, `nodes`, `plugins` e `ai`
...
- `converters` — normalização de fontes e geração de matriz/frames
- `core` / `ascii-interaction` — runtime sem UI, com influência, grid e física
- `animator` — reprodução, keyframes e operações temporais
- `browser` — clipboard, download e adaptação de I/O
- `gallery` + `icons` — biblioteca e conteúdo reutilizável do produto
- `presets` + `recipes` + `themes` — linguagem visual e presets de conversão
- `exporters` + `importers` — serialização e fluxo de projeto
- `nodes` + `plugins` — extensibilidade futura e registry de capacidades
- `ai` — stubs e integração futura, sem rede neste ciclo
- `storage` + `document` — persistência e serialização do estado do documento do projeto
